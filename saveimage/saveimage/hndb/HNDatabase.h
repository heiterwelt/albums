...
#import "HNResultSet.h"
#import "HNDatabasePool.h"


#if ! __has_feature(objc_arc)
    #define HNDBAutorelease(__v) ([__v autorelease]);
    #define HNDBReturnAutoreleased HNDBAutorelease

    #define HNDBRetain(__v) ([__v retain]);
    #define HNDBReturnRetained HNDBRetain

    #define HNDBRelease(__v) ([__v release]);

    #define HNDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define HNDBAutorelease(__v)
    #define HNDBReturnAutoreleased(__v) (__v)

    #define HNDBRetain(__v)
    #define HNDBReturnRetained(__v) (__v)

    #define HNDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define HNDBDispatchQueueRelease(__v)
    #else
        #define HNDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^HNDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);


/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in HNDB are:

 - `HNDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<HNResultSet>` - Represents the results of executing a query on an `HNDatabase`.
 - `<HNDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<HNDatabasePool>` - A pool of `HNDatabase` objects.
 - `<HNStatement>` - A wrapper for `sqlite_stmt`.
...
/** Execute select statement
 ...
 Executing queries returns an `<HNResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[HNResultSet next]>`) from one record to the other.
 ...
 @return A `<HNResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see HNResultSet
 @see [`HNResultSet next`](<[HNResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 
 @note If you want to use this from Swift, please note that you must include `HNDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeQuery:withArgumentsInArray:>`.
 */
...
- (HNResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
/** Execute select statement
 ...
 `func executeQuery(sql: String!, values: [AnyObject]!) throws  -> HNResultSet!`
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(HNDatabase *adb) {
        ...
        int rowCount = 0;
        HNResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            NSLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
        HNDBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [HNDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one HNDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using HNDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to HNDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using HNDB you will not need to interact directly with `HNStatement`, but rather with `<HNDatabase>` and `<HNResultSet>` only.
...
@interface HNStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
