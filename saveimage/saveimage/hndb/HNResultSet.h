...
/** Represents the results of executing a query on an `<HNDatabase>`.
...
@interface HNResultSet : NSObject {
    HNDatabase          *_parentDB;
    HNStatement         *_statement;
...
/** `HNStatement` used by result set. */

@property (atomic, retain) HNStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<HNStatement>`
 
 @param statement A `<HNStatement>` to be performed
 
 @param aDB A `<HNDatabase>` to be used
 
 @return A `HNResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(HNStatement *)statement usingParentDatabase:(HNDatabase*)aDB;

/** Close result set */

- (void)close;

- (void)setParentDB:(HNDatabase *)newDb;
