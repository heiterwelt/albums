...
+ (instancetype)resultSetWithStatement:(HNStatement *)statement usingParentDatabase:(HNDatabase*)aDB {
    ...
    return HNDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    HNDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return HNDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"HNDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
